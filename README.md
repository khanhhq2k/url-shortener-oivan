...
| **Scale + collision approach** in README | [Algorithm](#algorithm-and-uniqueness) and [Scaling](#scaling-and-the-collision-problem) below. You do **not** need a distributed “web scale” build for the challenge — behavior is documented honestly. |
...
You do not need PostgreSQL or Redis installed on the host.
...
Set **`PUBLIC_APP_ROOT`** in production if you need a stable public base URL for `shortened_link` (otherwise the request host is used).
...
- **Trade-off:** slugs correlate with insertion order; production mitigations include rate limits, abuse monitoring, longer random tokens — out of minimal assignment scope unless you extend.
...
2. **Redis server — eviction under memory cap:** for Redis you fully control, set **`maxmemory`** and **`maxmemory-policy allkeys-lru`**. **Docker Compose** runs local Redis with **`allkeys-lru`** and a **`maxmemory`** cap so cold keys drop first when full.
...
This demo doesn’t require implementing full large-scale infrastructure, but you should show you understand the bottleneck and what you’d do next.
...
- **Sharding / multi-region:** not implemented here; narrative stops at caching, replicas, and failover. (If you later need global ID allocation across shards, that becomes a coordinated design problem beyond this demo.)

In production you’d monitor Redis hit rate, miss latency, Redis evictions, and Postgres query time so you can resize/tune before cache-miss storms impact the primary.
...
- Deploy to any **free-tier host** you prefer; this repo is wired for **Docker** production image and documents **Fly.io** Redis in [SETUP.md](SETUP.md#flyio-redis-lru-and-eviction). Set app secrets such as **`DATABASE_URL`**, **`REDIS_URL`**, and optionally **`PUBLIC_APP_ROOT`** for stable short links.
